...
contract('dBank', ([deployer, user]) => {
  ...
  let balance
    ...
    describe('success', () => {
      ...
      beforeEach(async () => {
        await dbank.deposit({value: 10**18, from: user}) //1 ETH
        balance = await dbank.etherBalanceOf(user)
        await dbank.borrow({value: web3.utils.toWei(0.10), from: user}) //0.10 ETH

        borrowed = await dbank.collateralEther(user)
